...
| 경로 | 내용 |
|---|---|
| `mirae/config.py` | 분석 설정 — 대상어·경쟁 명명어·개념어 사전·평가어 사전·시기 구분·진영(camp) 배정 |
| `mirae/corpus.py` | TEI/XML 로더 (Kiwi 토큰화, `<quote>` 제외 텍스트 분리, 문장 단위 KWIC) |
| `mirae/analyses.py` | 로그우도·키워드성·담화 운율·명명 분산 |
| `run_mirae.py` | 전체 분석 실행 → 리포트 |
| `kwic_demo.py`, `prosody_demo.py` | KWIC·담화 운율 데모 |
| `h4_keyword_trajectory.py`, `per_year.py`, `periodize.py` | 통시 분석 |
| `render_tags.py`, `build_editor.py`, `build_html_by_period.py` | 태그 검수용 HTML 뷰어 생성 |
| `clean_corpus.py`, `ocr_audit.py`, `ocr_quality.py` | OCR 정제·품질 측정 |
| `schema/korean-critique-schema-v5.xsd` | 입력 TEI가 준수하는 도메인 스키마 (상위 저장소와 동일) |
| `examples/sample_2010_v2.xml` | 합성 태깅 예시 1건 (저작권 무관, 파이프라인 시험용) |
...
```bash
# 1) 예시로 파이프라인 동작 확인
python run_mirae.py            # 기본 SRC 경로를 examples/ 등 본인 코퍼스로 수정

# 2) 진영·시기·대상어는 mirae/config.py에서 조정
```
